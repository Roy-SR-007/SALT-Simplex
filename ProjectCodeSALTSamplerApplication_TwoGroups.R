...
library(extraDistr)
library(SALTSampler)
source("/Users/arhitchakrabarti/Desktop/Research/R codes/RunMH_my.R")
set.seed(2024)
...
beta1.start.dp = rep(1/L.max, L.max)
z1.start.dp = extraDistr::rcat(n = n1, prob = beta1.start.dp)

################################################################################
# GET estimate of beta1 by running DP
################################################################################
source("/Users/arhitchakrabarti/Desktop/Research/R codes/Blocked DP/DP_normal_functions.R")
num_iterations <- 10000
burn = 5000

init <- list(beta.start = beta1.start.dp,
             mu.start = mu.start,
             z.start = z1.start.dp)

prior <- list(alpha = alpha1.start,
              prior_mean = xi,
              prior_prec = lambda)


gibbs = gibbs_dp(init = init, data = X1, num_iterations = num_iterations,
                 burn = burn, L.max = L.max, prior = prior, samples = FALSE)

# Get the estimate of beta1 by running a Naive DP on group 1 data
beta1.start.small = gibbs$beta_post_mean
...
beta1.start.long = c(beta1.start.small, rep(1e-8, L.max - length(beta1.start.small)))
...
# @pars argument should be a named list with alpha2, beta2
# @ycurrent corresponds to the previous value of beta1
# @ycand corresponds to a proposed value of beta1
# @a corresponds to alpha1. 
# @dat corresponds to n1_h 's at any stage
Target_beta1 <- function(ycand, ycurrent, a, dat, pars) {
  ...
  alpha2 = as.numeric(pars$alpha2); beta2 = pars$beta2
  
  proposed = sum((dat + a/L - 1)*log(ycand) + (alpha2*ycand - 1)*log(beta2) - lgamma(alpha2 * ycand))
  previous = sum((dat + a/L - 1)*log(ycurrent) + (alpha2*ycurrent - 1)*log(beta2) - lgamma(alpha2 * ycurrent))
  out = proposed - previous
  return(out)
}  
...
for(b in 2:gibbs_iterations){
  ...
  beta1 <- RunMH_my(Target = Target_beta1, init = init, B = 1,
                    h = rep(2, L.obs), 
                    pars = list(beta2 = beta2_samples[[b-1]],
...
beta1.true
round(apply(beta1_samples_matrix, MARGIN = 2, FUN = mean), 4)
#####################################################################################
...
par(mfrow = c(1, 1))
plotpsm(sim_mat, ylab = "Observations", xlab = "Observations")
